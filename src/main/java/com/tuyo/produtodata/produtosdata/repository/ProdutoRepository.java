...
public interface ProdutoRepository extends PagingAndSortingRepository<Produto, Integer> {           // É preciso prover genérica informação que este repositório CRUD espera.
   ...
   // List<Produto> findByIdIn(List<Integer> ids);
    ...
    @Query(value = "CALL GetAllProducts", nativeQuery = true)                             // Anotação @Query. Passando dois parâmetros: (1). especificar que é uma native query. CALL = é como uma native query é chamada. (2. nativeQuery = true
    List<Produto> findAllProdutos();                      // Trabalhando com Store Procedures
}                                                          // O método poderia ser chamado de findAll_sp. sp = store procedure.
